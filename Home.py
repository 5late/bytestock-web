...
if st.button(label='Analyze'): # Analyze button
    ...
    with open('./bytestock-core-public/output.txt') as file:
        ...
        analyze_data = pd.DataFrame(list(zip(periods, probabilities)), columns=['Period (Number of days considered)', 'Probability']).reset_index(drop=True) # Dataframe for chart
